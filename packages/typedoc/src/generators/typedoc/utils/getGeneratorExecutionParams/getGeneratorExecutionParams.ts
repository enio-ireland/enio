...
export const getGeneratorExecutionParams = (tree: Tree, project: TypedocGeneratorSchema['project']): GeneratorExecutionParams => {
  ...
  if (isNotValidString(project))
    throwError(isMissingFromContext('Project name'), 'Review project name is correctly passed to typedoc command')

  const config = getProjectConfig(tree, project)

  if (isNotObject(config))
    throwError(isMissingFromContext(`Configuration for ${project}`), 'Check project.json file exists', reviewNxVersion)
