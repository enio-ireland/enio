...
describe('sameStructure', () => {
  ...
  it('returns matching data type for values with empty array', () => {
  ...
  it('returns matching data type for lists of same length', () => {
    expect(sameStructure(['apples', 'pears'], ['bananas', 'blueberries'])).toEqual('array')
  })

  it('returns matching data type for objects containing the same properties', () => {
    expect(sameStructure({ a: 0 }, { a: 100 })).toEqual('object')
    expect(sameStructure({ a: 0, b: '' }, { a: 100, b: false })).toEqual('object')
  })

  it('returns matching data type for objects containing the same properties (unordered)', () => {
...
describe('sameStructure - with config samePositionOfOwnProperties: true', () => {
  ...
  it('returns false for objects containing the same properties (unordered)', () => {
    expect(sameStructure({ b: -50, a: 'box' }, { a: 100, b: 'Fizz!' })).toEqual(false)
  })
})

describe('sameStructure - with extended iterable class types', () => {
  beforeEach(() => {
...
describe('sameStructure - with extended iterable class types and config samePositionOfOwnProperties: false', () => {
  beforeEach(() => {
    setConfig({ samePositionOfOwnProperties: false })
    registerIterableClass<Map<unknown, unknown>>(
      Map,
...
describe('sameStructure - with extended iterable class types and config samePositionOfOwnProperties: true', () => {
  beforeEach(() => {
